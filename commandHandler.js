const { Message } = require("elvan.js");
...
module.exports = (message) => {
    if (message.author.bot || message.channel.type == "dm" || !message.content.startsWith(Config.Prefix) || message.member.roles.cache.has(Settings.Penals.Jail.Role) || message.member.roles.cache.has(Settings.Roles.Unregistered)) return;
...
let args = message.content.split(" ").filter(arg => arg != " "), name = args[0];
 ...
 //                   if (diff < cooldown) return; message.reply(`Bu komutu tekrar kullanabilmek için **${Number(((cooldown - diff) / 1000).toFixed(2)).toHumanize({}, 3)} saniye** sonra tekrar dene.`);
